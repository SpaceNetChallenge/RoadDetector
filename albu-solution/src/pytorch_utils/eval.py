...
class Evaluator:
    def __init__(self, config, ds, folds, test=False, flips=0, num_workers=0, border=12):
        ...
        if self.config.save_images:
            os.makedirs(os.path.join(self.config.dataset_path, 'results'), exist_ok=True)
    ...
    def cut_border(self, image):
        if image.shape[:2] == (self.config.target_rows - 2 * self.border, self.config.target_cols - 2 * self.border):
            return image
        return image[self.border:-self.border, self.border:-self.border, ...]
