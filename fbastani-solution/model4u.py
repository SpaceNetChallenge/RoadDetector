...
class Model:
	...
	def __init__(self, in_channels=9, dropout=False):
		tf.reset_default_graph()

		self.is_training = tf.placeholder(tf.bool)
		self.inputs = tf.placeholder(tf.float32, [None, 256, 256, in_channels])
		self.targets = tf.placeholder(tf.float32, [None, 128, 128, 1])
		self.learning_rate = tf.placeholder(tf.float32)

		if dropout:
			dropout_factor = tf.to_float(self.is_training) * 0.5
		else:
			dropout_factor = None

		self.layer1 = self._conv_layer('layer1', self.inputs, 2, in_channels, 128, {'batchnorm': False}) # -> 128x128x128
		...
		self.layer5 = self._conv_layer('layer5', self.layer4, 2, 128, 256, {'dropout': dropout_factor}) # -> 32x32x256
		self.layer6 = self._conv_layer('layer6', self.layer5, 1, 256, 256, {'dropout': dropout_factor}) # -> 32x32x256
		self.layer7 = self._conv_layer('layer7', self.layer6, 2, 256, 512, {'dropout': dropout_factor}) # -> 16x16x512
		self.layer8 = self._conv_layer('layer8', self.layer7, 2, 512, 512, {'dropout': dropout_factor}) # -> 8x8x512
		self.layer9 = self._conv_layer('layer9', self.layer8, 1, 512, 512, {'dropout': dropout_factor}) # -> 8x8x512
		self.layer10 = self._conv_layer('layer10', self.layer9, 1, 512, 512, {'dropout': dropout_factor}) # -> 8x8x512
		self.layer11 = self._conv_layer('layer11', self.layer10, 1, 512, 512, {'dropout': dropout_factor}) # -> 8x8x512
		self.layer12 = self._conv_layer('layer12', self.layer11, 2, 512, 512, {'transpose': True, 'dropout': dropout_factor}) # -> 16x16x512
		self.layer13_inputs = tf.concat([self.layer7, self.layer12], axis=3)
		self.layer13 = self._conv_layer('layer13', self.layer13_inputs, 1, 1024, 512, {'dropout': dropout_factor}) # -> 16x16x512
		self.layer14 = self._conv_layer('layer14', self.layer13, 2, 512, 256, {'transpose': True, 'dropout': dropout_factor}) # -> 32x32x256
		self.layer15_inputs = tf.concat([self.layer6, self.layer14], axis=3)
		self.layer15 = self._conv_layer('layer15', self.layer15_inputs, 1, 512, 256, {'dropout': dropout_factor}) # -> 32x32x256
		...
		self.layer19 = self._conv_layer('layer19', self.layer19_inputs, 1, 256, 128) # -> 128x128x128
		self.layer20 = self._conv_layer('layer20', self.layer19, 1, 128, 128) # -> 128x128x128
		self.pre_outputs = self._conv_layer('pre_outputs', self.layer20, 1, 128, 2, {'activation': 'none', 'batchnorm': False}) # -> 128x128x2
