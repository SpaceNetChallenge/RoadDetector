...
class Model:
	...
	def __init__(self, in_channels=9):
		...
		self.inputs = tf.placeholder(tf.float32, [None, 1408, 1408, in_channels])
		self.targets = tf.placeholder(tf.float32, [None, 704, 704, 1])
		...
		self.layer5 = self._conv_layer('layer5', self.layer4, 2, 128, 256) # -> 32x32x256
		self.layer6 = self._conv_layer('layer6', self.layer5, 1, 256, 256) # -> 32x32x256
		self.layer7 = self._conv_layer('layer7', self.layer6, 2, 256, 512) # -> 16x16x512
		self.layer8 = self._conv_layer('layer8', self.layer7, 2, 512, 512) # -> 8x8x512
		self.layer9 = self._conv_layer('layer9', self.layer8, 1, 512, 512) # -> 8x8x512
		self.layer10 = self._conv_layer('layer10', self.layer9, 1, 512, 512) # -> 8x8x512
		self.layer11 = self._conv_layer('layer11', self.layer10, 1, 512, 512) # -> 8x8x512
		self.layer12 = self._conv_layer('layer12', self.layer11, 2, 512, 512, {'transpose': True}) # -> 16x16x512
		self.layer13_inputs = tf.concat([self.layer7, self.layer12], axis=3)
		self.layer13 = self._conv_layer('layer13', self.layer13_inputs, 1, 1024, 512) # -> 16x16x512
		self.layer14 = self._conv_layer('layer14', self.layer13, 2, 512, 256, {'transpose': True}) # -> 32x32x256
		self.layer15_inputs = tf.concat([self.layer6, self.layer14], axis=3)
		self.layer15 = self._conv_layer('layer15', self.layer15_inputs, 1, 512, 256) # -> 32x32x256
