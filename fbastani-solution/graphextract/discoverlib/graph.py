...
class Vertex(object):
	...
	def neighbors(self):
		...
		for edge in self.in_edges:
			n[edge.src.id] = edge.src
		for edge in self.out_edges:
			n[edge.dst.id] = edge.dst
		return n.values()
...
def shortest_distances_from_source(src):
	distances = {}
	remaining = set()
	seen = set()
	...
	while len(remaining) > 0:
		...
		for other in closestNode.neighbors():
...
def get_graph_road_segments(g):
	...
	def search_from_edge(rs, edge, direction):
		...
		edges = [next_edge for next_edge in edges if not next_edge.is_opposite(edge)]
		...
		search_from_edge(rs, next_edge, direction)
